// import buildTrajectoryFunction from "../utils/physics.js"
// import { submitShot } from "../utils/blockchain.js";
...
//         // Use a simple, reliable player image from Phaser's examples
//         this.load.image('player', 'https://labs.phaser.io/assets/sprites/phaser-dude.png');
        
//         // Use a simple ball for the projectile
...
//         // Setup player data with simpler names and tint for Player 2
//         this.players = [
...
//         // Create player sprites using simple images instead of animated sprites
//         this.players.forEach(p => {
//             p.sprite = this.add.image(p.x, p.y, 'player').setScale(1.5); // Using image instead of sprite
//             p.sprite.setTint(p.tint); // Tint Player 2 to differentiate
//         });

//         // Player 2 should face left
...
//         this.drawTrajectory(attacker.x, attacker.y - 20, this.currentVelocity, this.currentAngle); // Launch from slightly higher
//     }
...
//         let y = startY;
//         for (let t = 0; t < 60; t++) {
//             const newX = startX + t * velocity / 10 * Math.cos(angleRad);
...
//         const waitingText = this.add.text(400, 120, 'Waiting for chain...', { fontSize: '20px', color: '#ffaa00' }).setOrigin(0.5);
//         const { a, b, c, hit } = await submitShot({ velocity: this.currentVelocity, angle: this.currentAngle, target });
//         waitingText.destroy();
        
//         const trajectoryFn = buildTrajectoryFunction({ a, b, c });
//         this.animateProjectile(trajectoryFn, hit);
//     }

//     animateProjectile(trajectoryFn, hit) {
//         const attacker = this.players[this.currentPlayerId];
//         const defender = this.players[1 - this.currentPlayerId];
...
//         // Use the simple 'ball' asset for the projectile
...
//                 x += 5 * direction;
//                 projectile.x = x;
//                 projectile.y = trajectoryFn(x, startX, startY);

//                 const defenderRect = defender.sprite.getBounds();
//                 if (Phaser.Geom.Rectangle.Contains(defenderRect, projectile.x, projectile.y) || projectile.y > 600 || projectile.x < 0 || projectile.x > 800) {
